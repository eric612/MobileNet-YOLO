...
namespace caffe {
  ...
  template <typename Dtype>
  void GaussianYolov3Layer<Dtype>::LayerSetUp(
    ...
    int tmp_input_count = side_w_ * side_h_ * num_ * (8 + num_class_ + 1); //13*13*5*(20+4+1) label: isobj, class_label, coordinates
    ...
    for (int b = 0; b < bottom[0]->num(); b++) {
      ...
      for (int s = 0; s < stride; s++) {
        for (int n = 0; n < num_; n++) {
          ...
          int y2 = s / side_w_;
          int x2 = s % side_w_;
          get_gaussian_yolo_box(pred, swap_data, biases_, mask_[n], index, x2, y2, side_w_, side_h_, side_w_*anchors_scale_, side_h_*anchors_scale_, stride);
          for (int t = 0; t < 300; ++t) {
            vector<Dtype> truth;
            Dtype x = label_data[b * 300 * 5 + t * 5 + 1];
            Dtype y = label_data[b * 300 * 5 + t * 5 + 2];
